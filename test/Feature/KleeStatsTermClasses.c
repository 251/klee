...
// RUN: %klee --max-stack-frames=15 --switch-type=simple --search=dfs --use-merge --write-no-tests --output-dir=%t.klee-out %t.bc 2> %t.log
...
int main(void) {
  ...
  switch(cond) {
    ...
    case __LINE__:
      klee_make_symbolic(1);
    case __LINE__:
      __asm__("movq $60, %rax\n\t"
              "movq $2,  %rdi\n\t"
              "syscall"); // ExecErr (Exec)
    case __LINE__:
